...
SELECT txn_type,
        ...
        FORMAT(SUM(txn_amount), 'NO') AS total_amount
...
SELECT AVG(txn_amount) AS avg_txn_count,
...
-- Q4. What is closing balance for each customer at the end of the month?
...
-- CTE 2 - recursive_date       : Increment last days of each month till they are equal to @maxDate
...
WITH monthly_transactions AS (SELECT customer_id,
    ...
    recursive_dates AS (SELECT DISTINCT custmer_id,
...
-- Q5. What is the percentage of csutomers who increase their closing balance by more than 5%?
...
-- Create a temporary table becasue of the error: Null value is eliminated by an aggregate or other SET operation
SELECT *
INTO #temp
FROM pct_increase;

-- Calculate percentage of customers whose closing balance increasing 5% compared to the previous month
