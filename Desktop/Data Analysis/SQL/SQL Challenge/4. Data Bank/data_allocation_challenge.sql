-- running customer balance column that includes the impace each transaction
...
WITH amount_cte AS (SELECT customer_id,
    ...
    running_balance_cte AS (SELECT *,
                                    SUM(amount) OVER (PARTITION BY csutomer_id, txn_month ORDER BY customer_id, txn_month
                                                        ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_balance
                            FROM amount_cte
                        ),
...
-- Option 2: data is collected on the average amount of money kept in the account in the previous 30 days
WITH amount_cte AS (SELECT customer_id,
                            MONTH(txn_date) AS txn_month,
                            SUM(CASE
                                WHEN txn_type = 'deposit' THEN txn_amount
...
WITH amount_cte AS (SELECT customer_id,
    ...
    running_balance_cte AS (SELECT customer_id,
                                    ...
                                    SUM(balance) OVER (PARTITION BY customer_id ORDER BY txn_month
                                                        ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT RWOW) AS running_total
                            FROM amount_cte
                            GROUP BY customer_id,
                                        txn_month,
                                        balance
