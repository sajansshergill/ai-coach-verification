...
WITH customer_first_date AS (SELECT customer_id,
                                    node_id,
                                    region_id,
                                    SUM(DATEDIFF(DAY, start_date, end_date)) AS days_in node
                                FROM customer_nodes
                                WHERE end_date <> '9999-12-31'
                                GROUP BY customer_id,
                                            node_id,
...
SELECT DISTINCT r.region_id,
                ...
                PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY r.days_in_node) OVER (PARTITION BY r.region_id) AS pecentile_95
