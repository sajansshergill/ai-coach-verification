...
WITH product_info AS (SELECT ph.product_id,
    ...
    product_abadoned AS (SELECT ph.product_id,
                                ph.page_name AS product_name,
                                ph.product_category,
                                COUNT(*) AS abandoned
                        FROM events AS e
...
FROM product_infor AS pi
    JOIN product_abadoned AS pa
        ON pi.product_id = pa.product_id
    JOIN product_abadoned AS pp
...
-- Q4. What is the average conversion rate from view to cart add?
SELECT CAST(AVG(100.0 * cart_ads / views) as DECIMAL(5,2)) AS avg_view_to_cart
FROM #product_summary;
