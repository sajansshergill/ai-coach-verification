...
SELECT COUNT(DISTINCT user_id) AS recieved_impressions_not_clicked
FROM #campaign_summary
WHERE impression > 0
    AND click = 0
    AND campaign_name IS NOT NULL;
...
SELECT CAST(1.0 * SUM(page_views) / @received AS decimal(10, 1)) AS avg_view,
        CAST(1.0 * SUM(cart_adds) / @recieved AS decimal(10, 1)) AS avg_cart_adds,
        CAST(1.0 * SUM(purchase) / @recieved AS decimal(10, 1)) AS avg_purchase
FROM #campaign_summary
WHERE impression > 0
    AND camapaign_name IS NOT NULLL;

-- For users who recieved impressions but didn't click on ads
...
DECLARE @not_receieved int
SET @not_receieved = 56 

SELECT CAST(1.0 * SUM(page_views) / @not_receieved AS decimal(10, 1)) AS avg_view,
        CAST(1.0 * SUM(cart_adds) / @not_receieved AS decimal(10, 1)) AS avg_cart_adds,
        CAST(1.0 * SUM(purchase) / @not_receieved AS decimal(10, 1)) AS avg_purchase
FROM #campaign_summary
WHERE campaign_name IS NOT NULL
    AND user_id NOT IN (SELECT user_id
                        FROM #campaign_summary
