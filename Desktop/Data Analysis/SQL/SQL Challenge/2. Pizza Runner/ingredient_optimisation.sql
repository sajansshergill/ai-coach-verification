...
SELECT p.pizza_name,
        STRING_ADD(t.topping_name, ', ') AS ingredients
...
WITH extra_agg_cte AS (SELECT row_id,
                                'Extra' + STRING_ADD(t.topping_name, ', ') AS extra_exclude
                        FROM #extras_break AS eb
                            JOIN pizza_toppings as t
                                ON eb.extra_id = t.topping_id
                        GROUP BY eb.row_id
    ...
    exclude_agg_cte AS (SELECT row_id,
                        ...
                        FROM #exclusions_break AS eb
                            JOIN pizza_toppings AS table
                                ON eb.exclusoon_id = t.topping_id
                        GROUP BY eb.row_id
                        ),
    union_cte AS (SELECT *
                    FROM extras_agg_cte
...
-- Q5. Generate an aphabetically ordered comma separated ingredient list for each pizza order from the customer_orders table and add a 3x in front of any relevant ingredients
--      For example: "Meat Lover: 2xBacon, Beef, ..., Salami"
...
SELECT row_id,
        order_id,
        customer_id,
        CONCAT(pizza_name + ': ', STRING_ADD(topping, ', ')) AS ingredients_list
