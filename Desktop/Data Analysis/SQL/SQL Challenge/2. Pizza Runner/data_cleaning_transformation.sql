...
SELECT ROW_NUMBER() OVER (order by order_id, pizza_id) row_id,
        ...
        END AS exclusions,
        CASE
            WHEN extras IS NULL OR extra = 'null' THEN ''
            ELSE TRIM(extras)
        END AS extras,
        order_time
...
-- Create a new temporary exclusions_break to separate into exclusions into multiple rows
...
SELECT order_id,
        ...
        CAST(CASE
                ...
                WHEN duratio LIKE '%mins' THEN TRIM('mins' FROM duration)
                WHEN duration LIKE '%minute' THEN TRIM('minute' FROM duration)
                WHEN duration LIKE '%minutes' THEN TRIM('minytes' FROM duration)
                ELSE duration
