...
-- Q1. How many runneres signed up for each 1 week period? (I.E. week starts 2021-01-01)
...
FROM (SELECT c.order_id,
            ...
            DATEDIFF(MINUTE, c.order_time, r.pickup_tim) AS prep_time
    FROM #customer_orders_temp AS c
        JOIN #runner_orders_temp AS r
            ON c.order_id = r.order_id
    WHERE r.duration <> 0
...
-- Q5. What was the difference betweem the longest and shortest delivery times for all orders?
...
-- Q7. What is successful delivery percentage for each runner?
SELECT runner_id,
        SUM(CASE
