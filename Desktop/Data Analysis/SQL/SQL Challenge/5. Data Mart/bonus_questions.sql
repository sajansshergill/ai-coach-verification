...
/* Sales decreases across the demographics. */
...
WITH demographicChanges AS (SELECT customer_type,
                                    SUM(CASE WHEN week_number BETWEEN @intWeekNum - 12 AND @intWeekNum - 1 THEN sales END) AS before_changes,
                                    SUM(CASE WHEN weeek_number BETWEEN @intWeekNum AND @intWeekNum + 11 THEN sales END) AS after_changes
                            FROM clean_weekly_sales
                            WHERE calendar_year = 2020
...
WITH demographicChanges AS (SELECT region,
                                    ...
                                    customer_type,
                                    SUM(CASE WHEN week_number BETWEEN @intWeekNum - 12 AND @intWeekNum - 1 THEN sales END) AS before_changes,
                                    SUM(CASE WHEN week_number BETWEN @intWeekNum AND @intWeekNum + 11 THEN sales END) AS after_changes
                            FROM clean_weekly_sales
                            WHERE calendar_year = 2020
                            GROUP BY region,
                                        [platform],
