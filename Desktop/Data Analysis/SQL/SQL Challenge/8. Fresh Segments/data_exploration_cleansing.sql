-- Q1. Update the fresh_segments.interest_metrics table by modifying the month_year column to be a date type with the start of the month
UPDATE interest_metrics
SET month_year = CONVERY(DATE, '01-' + CONCAT(VARCHAR(10), month_year), 105);
...
-- The NULL values appears in "_month", "_year", "month_year", "interest_id" with the exeption of interest_id 21246.
SELECT *
FROM interest_metrics
WHERE month_year IS NULL;

-- Before take any action (drop, fill...), it would be useful to check the percentage of NULL values
...
-- Q4. How many interest_id values exost in the fresh_segments.interest_metrics table but not in the fresh_segments.interest_map table?
...
-- Q5. Summarise the id values in the fresh_segements.interest_map by its total record count in this table
SELECT COUNT(*)
FROM interest_map;
...
--      and include all columns from fresh_segments.interest_metrics and all columns from fresh_segement.interest_map except from the id column.
SELECT *
...
-- Q7. Are there any records in your joined table where the month_year value is before the created_at value from the fresh_segments.interes_map table?
...
--      The reason is becasue month_year values were set on the first_date of the month by default in Question 1
