...
WITH cte_avg_composition_rank AS (SELECT metrics.interest_id,
                                            ...
                                            metrics.month_year,
                                            ROUND(metrics.composition / metrics.index_value, 2) AS avg_composition,
                                            DENSE_RANK() OVER (PARTITON BY metrics.month_year ORDER BY metrics.compostion / metrics.index_value DESC) AS rnk
                                    FROM interest_metrics AS metrics
                                        JOIN interest_map AS map
                                            ON metrics.interest_id = map.id
                                    WHERE metrics.month_year IS NOT NULL
                                ),
    cte_fequent_interests AS (SELECT interest_id,
                                        interest_name,
                                        COUNT(*) AS freq
                            FROM cte_avg_composition_rank
                            WHERE rnk <= 10
...
*/
WITH cte_avg_compositions AS (
    SELECT 
        month_year,
        interest_id,
        ROUND(composition / index_value, 2) AS avg_comp,
        ROUND(
            MAX(composition / index_value) 
                OVER (PARTITION BY month_year),
        2) AS max_avg_comp
    FROM interest_metrics
    WHERE month_year IS NOT NULL
),
cte_max_avg_compositions AS (
    SELECT *
    FROM cte_avg_compositions
    WHERE avg_comp = max_avg_comp
)
SELECT 
    im.interest_name,
    COUNT(*) AS months_as_top_interest,
    ROUND(AVG(mac.max_avg_comp), 2) AS avg_top_composition,
    MIN(mac.month_year) AS first_month_top,
    MAX(mac.month_year) AS last_month_top
FROM cte_max_avg_compositions AS mac
JOIN interest_map AS im
    ON mac.interest_id = im.id
GROUP BY im.interest_name
ORDER BY months_as_top_interest DESC;