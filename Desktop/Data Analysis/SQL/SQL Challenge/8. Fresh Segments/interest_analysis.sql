...
SELECT interest_id,
        COUNT(month_year) AS unique_month_year_count
FROM interest_metrics
...
--      - whoch total_months values passes the 90% cumulative percentage value?
WITH cte_interest_months AS (SELECT interest_id,
    ...
    cte_interest_counts AS (SELECT total_months,
                                    COUNT(DISTINCT interest_id) AS cte_interest_counts
...
-- Q3. If we were to remove all interest_id values whihc are lower than the total_monthe value we found in the previous question
--      - how many total data points would be removing?
WITH cet_interest_months AS (SELECT interest_id,
                                    COUNT(DISTINCT month_year) AS total_months
                            FROM interest_metrics
                            WHERE interest_id IS NOT NULL
                            GROUP BY interest_id
...
WHERE interest_id IN (SELECT interest_id
                        FROM cet_interest_months
                        WHERE total_months < 6);

-- Q4. Does this decision make sense to remove these data points from a business perspective?
--  Use and example where there are all 14 months present to a removed interest example for your arguments
--      - think about what it means to have less months presents from a segement perspective.
SELECT MIN(month_year) AS first_date,
        MAX(month_year) AS last_date
FROM interest_metrics;

SELECT month_year,
        COUNT(DISTINCT interes_id) interest_count,
