...
SELECT TOP 1 WITH TIES m.customer_id, mn.product_name
FROM members AS m
    JOIN Sales AS s
...
SELECT customer_id, product_name
FROM (SELECT m.customer_id, mn.product_name,
...
WITH customer_data AS (SELECT s.custome_id,
                        s.order_date,
                        mn.product_name,
                        mn.price,
                        CASE
