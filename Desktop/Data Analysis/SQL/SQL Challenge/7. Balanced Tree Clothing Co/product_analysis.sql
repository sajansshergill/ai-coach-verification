...
-- Q2. What is the total quantity, revenue and discount for each segement?
...
-- Q4. What is the total quantity, revenue and discount fro each category?
...
WITH top_selling_cte AS (SELECT pd.category_name,
                        ...
                        FROM sales AS s
                            JOIN product_details AS product_details
                                ON s.prod_id = pd.product_id
                        GROUP BY pd.product_name,
                                    pd.category_name
                        )
...
WITH segmen_prod_rev_cte AS (SELECT pd.segment_name,
                                    pd.product_name,
...
SELECT segment_name,
        product_name,
        CAST(100.0 * prod_revenue / SUM(prod_revenue) OVER (PARTITION BY segement_name) AS DECIMAL(5,2)) AS pct_segment_prod
...
WITH prods_per_txn_cte AS (SELECT s.txn_id,
    ...
    combination_count_cte AS (SELECT product_ids,
                                ...
                                GROUP BY product_ids, product_name
                            )
SELECT product_ids, product_names
FROM combination_count_cte
WHERE common_combinations = (SELECT MAX(common_combinations) FROM combination_count_cte);